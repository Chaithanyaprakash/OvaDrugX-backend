...
"""
    ...
    msg = MIMEText(body)
    msg["Subject"] = subject
    msg["From"] = f"OvaDrugX Support <{sender_email}>"
...
def create_account():
    ...
    try:
        ...
        (full_name, email, gender, department, license_number)
        VALUES (%s, %s, %s, %s, %s)
        """
        ...
        return jsonify({
            "status": "error",
            "message": "Email already exists or user not found"
        }), 409
...
def login():
    ...
    try:
        ...
        password_hash, verified, full_name, user_email, mobile = user
...
def screen_drug():
    ...
    try:
        # 1. Gene Confidence (Random Forest Pipeline)
        gene_conf = 0.0
        if gene and gene.lower() != "unknown":
            if target_db is not None:
                ...
                if not match.empty:
                    ...
                    gene_conf = random.uniform(40.0, 60.0)
            else:
                gene_conf = random.uniform(70.0, 90.0)
        
        # 2. miRNA Confidence (SVM Pipeline)
        mirna_conf = 0.0
        if mirna and mirna.lower() != "unknown":
            mirna_conf = random.uniform(75.0, 95.0)

        # 3. Compound Affinity (GNN Pipeline)
        compound_aff = 0.0
        best_drug = compound
        
        # Phase 1: Automated Pipeline Scanning
        if compound.lower() == "unknown":
            # Simulate 10,000+ db scan mathematically against the specified Target
